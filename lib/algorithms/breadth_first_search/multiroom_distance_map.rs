...
pub fn bfs_multiroom_distance_map(
    start: Vec<Position>,
    get_cost_matrix: impl Fn(RoomName) -> &'static ClockworkCostMatrix,
    _max_tiles: usize,
    _max_rooms: usize,
    ...
    while let Some(position) = frontier.pop_front() {
        ...
        for neighbor in neighbors(position).into_iter() {
            let cost_matrix = cost_matrices
                .entry(neighbor.room_name())
                .or_insert_with(|| get_cost_matrix(neighbor.room_name()));
...
pub fn js_bfs_multiroom_distance_map(
    ...
    bfs_multiroom_distance_map(
        ...
        |room| {
            ...
            match result {
                Ok(value) => <&'static ClockworkCostMatrix>::from(value),
                Err(e) => throw_str(&format!("Error getting cost matrix: {:?}", e)),
            }
        },
        max_tiles,
        max_rooms,
